...
describe("/api/articles", () => {
  describe("GET", () => {
    ...
    describe("Default response", () => {
      ...
      test("has [body.articles] with 5 valid items", () => {
        return request(app)
          .get("/api/articles")
          .then((res) => {
      ...
      test("[body.articles] is sorted by [created_at] in descending order", () => {
        return request(app)
          .get("/api/articles")
          .then((res) => {
            const { articles } = res.body;
            const isSortedByDate = customSort(
              articles,
              "created_at",
              "DESC",
              "time"
      ...
      test("has [body.pagination] with exact details", () => {
        return request(app)
          .get("/api/articles")
          .then((res) => {
            ...
            expect(pagination).toEqual(output);
          });
      });
    });
